...
This repository supports label smoothing through `--label_smoothing`.

- `--label_smoothing 0.1`: label smoothing with \(\epsilon_{ls}=0.1\).
- `--label_smoothing 0.0`: standard cross-entropy target (no smoothing).

Run with label smoothing (`ε_ls=0.1`):

```python
!python train.py \
  --project da6401-a3 \
  --task task2.5 \
  --run_name task2_5_ls_0p1 \
  --scheduler noam \
  --attention_scaling scaled \
  --positional_encoding sinusoidal \
  --label_smoothing 0.1 \
...
Run without label smoothing (`ε_ls=0.0`):

```python
!python train.py \
  --project da6401-a3 \
  --task task2.5 \
  --run_name task2_5_ls_0p0 \
  --scheduler noam \
  --attention_scaling scaled \
  --positional_encoding sinusoidal \
  --label_smoothing 0.0 \
...
### What to plot for Task 2.5

Use W&B overlays for the two runs:

- **Main required plot:** x-axis `epoch`, y-axis `val/prediction_confidence`.
- **Support plot:** x-axis `epoch`, y-axis `train/prediction_confidence`.
- **Support plot:** x-axis `epoch`, y-axis `val/bleu`.
- **Support plot:** x-axis `epoch`, y-axis `val/loss`.

`prediction_confidence` is logged as the average softmax probability assigned to the correct target token (ignoring `<pad>`).

### Reflection points for your report

- Label smoothing is a regularizer: it replaces one-hot targets with a softer target distribution, reducing pressure to place near-probability-1 mass on a single token.
- This discourages over-confident predictions and generally improves calibration and robustness.
- Because probability mass is intentionally spread, smoothed models can have higher training perplexity than hard-target training, even when they generalize better on validation metrics such as BLEU.
